...
RMenu.Add('example', 'main', RageUI.CreateMenu("", "~p~KTP"))
RMenu.Add('example', 'chargeur', RageUI.CreateSubMenu(RMenu:Get('example', 'main'), "", "~p~Balinese - KTP"))

Citizen.CreateThread(function()
    while true do
        RageUI.IsVisible(RMenu:Get('example', 'main'), true, true, true, function()

            RageUI.Button("KTP", "Dapatkan KTP", {RightLabel = "~g~100$"}, true, function(Hovered, Active, Selected)
                if (Selected) then
                    TriggerServerEvent('bir:ktp')
                      end
                end)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            for k in pairs(position) do
                ...
                if dist <= 3.0 then
                    ESX.ShowHelpNotification("Tekan ~INPUT_CONTEXT~ Untuk Membeli ~p~KTP")
...
Citizen.CreateThread(function()
    while true do
        ...
        if Vdist2(GetEntityCoords(PlayerPedId(), false), v1) < distance then
            Draw3DText(v1.x,v1.y,v1.z, "Om Sutejo")
        end
    end
end)
